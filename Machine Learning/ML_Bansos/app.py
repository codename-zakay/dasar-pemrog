...
if uploaded_file is not None:
    try:
        ...
        # Parameter model
        test_size = st.sidebar.radio("Ukuran Data Testing (%)", [10, 20, 30, 40]) / 100
        n_estimators = st.sidebar.radio("Jumlah Pohon (n_estimators)", 50, 500, 100, 50)
        max_depth = st.sidebar.radio("Kedalaman Maksimal (max_depth)", 3, 20, 7)
        random_seed = st.sidebar.radio("Random Seed", 1, 100, 42)
    ...
    with col1:
        st.caption("© 2024 Dinas Sosial")
    with col2:
        st.caption("v1.0.0")
    with col3:
        st.caption("Powered by Streamlit + Random Forest")